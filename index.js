    ...
    checkRGB(node) {
        if (this.state.denyRGB) {
            if (node.key && /^rgb(a)?$/i.test(node.key)) {
                let fix = '';
                const hex = this.RGBToHex(parseInt(node.nodes[0] ? node.nodes[0].toString() : '0', 10), parseInt(node.nodes[1] ? node.nodes[1].toString() : '0', 10), parseInt(node.nodes[2] ? node.nodes[2].toString() : '0', 10));
                if (node.key === 'rgb') {
                    fix = hex;
                }
                else {
